...
export const videos = ({ videos: { results } }, { filter }) =>
...
export const createdBy = (parent, _, { dataSources }, info) =>
  Promise.all(
    parent.created_by.map(({ id }) => dataSources.MovieDB.person({ id }, info))
  )

export const cast = ({ credits }, { limit }) =>
  credits.cast.slice(0, limit ? limit : cast.length) || []

export const crew = ({ credits }, { limit }) =>
  Object.values(
...
export const networks = (parent, _, { dataSources }, info) =>
  Promise.all(
    parent.networks.map(({ id }) => dataSources.MovieDB.network({ id }, info))
  )

export const productionCompanies = (parent, _, { dataSources }, info) =>
  Promise.all(
...
export const episodes = async (parent, _, { dataSources }, info) => {
  ...
  const results = await Promise.all(
    fetchedSeasons.map(({ season_number: season, episodes: eps }) =>
      eps.map(({ episode_number: episode }) =>
  ...
  return results
    ...
    .map(episode => ({ ...episode, series: parent }))
}
...
export const similar = ({ similar: { results } }, { limit }) =>
