const mapResultsToMovies = (results, dataSources, info) =>
  Promise.all(results.map(({ id }) => dataSources.MovieDB.movie({ id }, info)))
const mapResultsToPeople = (results, dataSources, info) =>
  Promise.all(results.map(({ id }) => dataSources.MovieDB.person({ id }, info)))
const mapResultsToShows = (results, dataSources, info) =>
  Promise.all(results.map(({ id }) => dataSources.MovieDB.tv({ id }, info)))

export const movie = (parent, { id }, { dataSources }, info) =>
  dataSources.MovieDB.movie({ id }, info)

export const person = (parent, { id }, { dataSources }, info) =>
  dataSources.MovieDB.person({ id }, info)

export const tv = (parent, { id }, { dataSources }, info) =>
...
export const search = async (
  ...
  const resolved = await Promise.all([
    ...
    mapResultsToPeople(
      results.filter(result => !result.media_type),
      dataSources,
      info
    ),
    mapResultsToShows(
...
export const airingThisWeek = async (
  parent,
  { page = 1 },
  { dataSources },
  info
